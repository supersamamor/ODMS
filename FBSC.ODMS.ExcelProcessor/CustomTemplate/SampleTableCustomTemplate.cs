...
namespace FBSC.ODMS.ExcelProcessor.CustomTemplate
{
    /// <summary>
    ...
    public static class SampleTableCustomTemplate
    {
        ...
        {
            IList<TemplateStructure> templateStructure = 
            [
        ...
        public static TableObject CreateRecord<TableObject>(Dictionary<string, object?> rowValue) where TableObject : new()
        {
            ...
            Type sampleTableStateType = typeof(SampleTableState);    
            foreach (var propertyName in Generate())
            {
                switch (propertyName.Name)
                {
                    case SampleField1:
                        PropertyInfo? sampleField1PropertyInfo = sampleTableStateType.GetProperty(nameof(SampleTableState.SampleField1));
                        sampleField1PropertyInfo!.SetValue(successRecord, rowValue[nameof(SampleTableState.SampleField1)]);
                        break;
                    case SampleField2:
                        PropertyInfo? sampleField2PropertyInfo = sampleTableStateType.GetProperty(nameof(SampleTableState.SampleField2));
                        sampleField2PropertyInfo!.SetValue(successRecord, DateTime.Now);
                        break;
                    case SampleField3:
                        PropertyInfo? sampleField3PropertyInfo = sampleTableStateType.GetProperty(nameof(SampleTableState.SampleField3));
                        sampleField3PropertyInfo!.SetValue(successRecord, 12345.67);
                        break;
                    case SampleField4:
                        PropertyInfo? sampleField4PropertyInfo = sampleTableStateType.GetProperty(nameof(SampleTableState.SampleField4));
