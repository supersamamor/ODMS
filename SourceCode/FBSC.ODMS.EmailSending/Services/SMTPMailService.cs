...
namespace FBSC.ODMS.EmailSending.Services
{
    public class SMTPEmailService(IOptions<MailSettings> settings, IConfiguration Configuration) : IMailService
	{
		...
		{  
			...
			if (_settings.PasswordEncrypted)
			{
				password = Common.Utility.Helpers.EncryptionHelper.DecryptPassword(_settings.SMTPEmailPassword!, _settings.SMTPEmail!);
			}
			else
			{
				password = _settings.SMTPEmailPassword!;
			}			
			...
			try
			{
				...
				if (!string.IsNullOrEmpty(_settings.SMTPEmailPassword))
				{
					...
					client.Credentials = new System.Net.NetworkCredential(_settings.SMTPEmail!, password);
				}
				else
				{
