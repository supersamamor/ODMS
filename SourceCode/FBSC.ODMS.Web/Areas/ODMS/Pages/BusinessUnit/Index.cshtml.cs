...
{   
    ...
    {        
        return await BatchUploadAsync<IndexModel, BusinessUnitState>(BatchUpload.BatchUploadForm, nameof(BusinessUnitState), "Index");
    }

    public IActionResult OnPostDownloadTemplate()
    {
        ModelState.Clear();
		BatchUpload.BatchUploadFileName = ExcelService.ExportTemplate<BusinessUnitState>(_uploadPath + "\\" + WebConstants.ExcelTemplateSubFolder);
