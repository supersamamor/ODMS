...
{   
    ...
    {
		var result = await Mediatr.Send(DataRequest!.ToQuery<GetDataUploadQuery>());
    ...
    {        
        return await BatchUploadAsync<IndexModel, DataUploadState>(BatchUpload.BatchUploadForm, nameof(DataUploadState), "Index");
    }

    public IActionResult OnPostDownloadTemplate()
    {
        ModelState.Clear();
		BatchUpload.BatchUploadFileName = ExcelService.ExportTemplate<DataUploadState>(_uploadPath + "\\" + WebConstants.ExcelTemplateSubFolder);
