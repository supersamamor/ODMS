...
{   
    ...
    {
		var result = await Mediatr.Send(DataRequest!.ToQuery<GetMilestoneQuery>());
    ...
    {        
        return await BatchUploadAsync<IndexModel, MilestoneState>(BatchUpload.BatchUploadForm, nameof(MilestoneState), "Index");
    }

    public IActionResult OnPostDownloadTemplate()
    {
        ModelState.Clear();
		BatchUpload.BatchUploadFileName = ExcelService.ExportTemplate<MilestoneState>(_uploadPath + "\\" + WebConstants.ExcelTemplateSubFolder);
