namespace FBSC.ODMS.Web
{
    public static class WebConstants
    {
		public const string SecureUploadFilePath = "SecureUploadFilePath";
        public const string PublishedUploadFilePath = "PublishedUploadFilePath";
		public const string ViewAttachmentPath = "ViewAttachment";
		public const string ViewFilePath = "ViewFile";
		public const string ExcelTemplateSubFolder = "ExcelTemplate";
		public const string BatchUpload = "BatchUpload";
		public const string DataTableDOMLayOut = @"<""top row""<""col-12 col-lg-8 custom-search-filters-container""<""row""<""filter-group""f><""filter-group""><""filter-group""><""filter-group""><""filter-group""><""filter-group"">>><""col-12 col-lg-4 custom-toolbar-container""B>>rt<""bottom row""<""col-12 col-md-6""i><""col-12 col-md-6 fills-pagination-container""p>><""clear"">";
		public const string DashboardDataTableDOMLayOut = @"<""top row""<""col-6 custom-search-container""f><""col-6 custom-toolbar-container""B>>rt<""bottom row""<""col-12 text-end fills-pagination-container""p><""col-12 text-end""i>><""clear"">";
		public const string ReportQueryAIGenerationInstruction = "Please provide a detailed description of the report, and if possible, include all the fields you would like to see. This will greatly enhance the AI's ability to accurately fulfill your request.";
		public const string DataUpload = "DataUpload";
		public const string DataSource = "DataSource";
		public const string Project = "Project";
		// Sub-folder (fieldName) under the project's secure-upload directory where
		// Statement-of-Work attachments are stored: {module}/{projectId}/{ProjectSowField}/{file}.
		// Also the fieldName used to build ViewAttachment download links.
		public const string ProjectSowField = "SOW";

    }
}
