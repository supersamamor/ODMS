...
namespace FBSC.Common.Core.Commands;
...
public abstract class BaseCommandHandler<TContext, TEntity, TCommand>(TContext context,
                          IMapper mapper,
    ...
    where TCommand : IEntity
{
    ...
    /// </summary>
    protected readonly CompositeValidator<TCommand> Validators = validator;
	...
	where TSubCollection : class
	{
        ...
        if (collectionAccessor(entity) is IEnumerable<TSubCollection> collection && collection.Any())
        {
