...
public abstract class AuditableDbContext<T>(DbContextOptions<T> options,
                        IAuthenticatedUser authenticatedUser) : AuditableContext(options) where T : DbContext
{
    /// <summary>
    /// Represents the authenticated user doing the transaction.
    /// </summary>
    protected readonly IAuthenticatedUser AuthenticatedUser = authenticatedUser;
	...
	void SetBaseFieldsStateless()
	{
		foreach (var entry in ChangeTracker.Entries<BaseEntity>())
			...
			{
				case EntityState.Added:
					entry.Entity.Entity = "Default";
					entry.Entity.CreatedDate = DateTime.UtcNow;
					entry.Entity.CreatedBy = "N/A";
					entry.Entity.LastModifiedDate = DateTime.UtcNow;
					entry.Entity.LastModifiedBy = "N/A";
					break;
				case EntityState.Modified:
					entry.Entity.LastModifiedDate = DateTime.UtcNow;
					entry.Entity.LastModifiedBy = "N/A";
