...
namespace FBSC.Common.Web.Utility.Logging;
...
public static class LogEnricher
{
    ...
    {
        diagnosticContext.Set("ClientIP", httpContext.Connection.RemoteIpAddress == null ? string.Empty : httpContext.Connection.RemoteIpAddress!.ToString());
        ...
        diagnosticContext.Set("Claims", httpContext.User == null ? string.Empty : httpContext.User);
        ...
        if (httpContext.Request.ContentLength.HasValue && httpContext.Request.ContentLength > 0)
        {
            if (httpContext.Request.ContentType!.Contains("form", System.StringComparison.InvariantCultureIgnoreCase))
            {
                ...
                if (form != null)
                {
                    if (form.ContainsKey("Input.Password"))
                    {
                        form["Input.Password"] = "[redacted]";
                    }
                    if (form.ContainsKey("Input.ConfirmPassword"))
                    {
                ...
                {
                    diagnosticContext.Set("Form", new Dictionary<string, Microsoft.Extensions.Primitives.StringValues>());
                }
                diagnosticContext.Set("FormFiles", httpContext.Request.Form?.Files ?? new FormFileCollection());
            }
        ...
        diagnosticContext.Set("TraceId", Activity.Current?.Id ?? httpContext?.TraceIdentifier ?? string.Empty);
    }
}