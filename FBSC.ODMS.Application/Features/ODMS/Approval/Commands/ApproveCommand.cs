...
public class ApproveCommandHandler(ApplicationContext context, IAuthenticatedUser authenticatedUser) : IRequestHandler<ApproveCommand, Validation<Error, ApprovalResult>>
{
    ...
    public async Task<Validation<Error, ApprovalResult>> Approve(ApproveCommand request, CancellationToken cancellationToken)
    {
        var entity = await (from a in _context.Approval
					...
					&& b.ApproverSetup!.TableName == request.Module
