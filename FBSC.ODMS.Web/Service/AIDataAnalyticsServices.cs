...
{
    public class AIDataAnalyticsServices(WebhookExecutionService webhookExecutionService, AiResponseParserFactory aiResponseParserFactory)
    {
        ...
        public async Task<string?> AIDrivenAnalysis(string reportName, string reportData, string reportColumns, string? customInstruction = null, CancellationToken token = new CancellationToken())
        {
			...
			var result = await webhookExecutionService.ExecuteWebhooksWithResponseAsync(WebhookEvents.PromptGenerativeAI, reportName, null,
				customQueryParameters: customQueryParameters, cancellationToken: token);            
