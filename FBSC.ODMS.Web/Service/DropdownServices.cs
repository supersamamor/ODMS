...
namespace FBSC.ODMS.Web.Service
{
    public class DropdownServices(ApplicationContext context, IMediator mediaTr)
    {
        ...
        public SelectList GetDashboardQueryList(string? id)
		{
			return _context.GetSingle<DashboardQueryState>(e => e.Id == id, new()).Result.Match(
				Some: e => new SelectList(new List<SelectListItem> { new() { Value = e.Id, Text = e.QueryHash } }, "Value", "Text", e.Id),
				None: () => new SelectList(new List<SelectListItem>(), "Value", "Text")
			);
		}
		public SelectList GetDashboardThemeList(string? id)
		{
			return _context.GetSingle<DashboardThemeState>(e => e.Id == id, new()).Result.Match(
				Some: e => new SelectList(new List<SelectListItem> { new() { Value = e.Id, Text = e.Code } }, "Value", "Text", e.Id),
				None: () => new SelectList(new List<SelectListItem>(), "Value", "Text")
			);
		}
		public SelectList GetDashboardList(string? id)
		{
			return _context.GetSingle<DashboardState>(e => e.Id == id, new()).Result.Match(
				Some: e => new SelectList(new List<SelectListItem> { new() { Value = e.Id, Text = e.Code } }, "Value", "Text", e.Id),
				None: () => new SelectList(new List<SelectListItem>(), "Value", "Text")
			);
		}
		public SelectList GetReportTypeList(string? id)
		{
			return _context.GetSingle<ReportTypeState>(e => e.Id == id, new()).Result.Match(
				Some: e => new SelectList(new List<SelectListItem> { new() { Value = e.Id, Text = e.Code } }, "Value", "Text", e.Id),
				None: () => new SelectList(new List<SelectListItem>(), "Value", "Text")
			);
		}
		public SelectList GetDataSourceList(string? id)
		{
			return _context.GetSingle<DataSourceState>(e => e.Id == id, new()).Result.Match(
				Some: e => new SelectList(new List<SelectListItem> { new() { Value = e.Id, Text = e.Name } }, "Value", "Text", e.Id),
				None: () => new SelectList(new List<SelectListItem>(), "Value", "Text")
			);
		}
		public SelectList GetDataUploadBatchList(string? id)
		{
			return _context.GetSingle<DataUploadBatchState>(e => e.Id == id, new()).Result.Match(
