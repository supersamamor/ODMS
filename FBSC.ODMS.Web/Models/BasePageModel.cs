...
public class BasePageModel<T> : PageModel where T : class
{
    ...
    private PageResult PageWithErrors(Seq<Error> errors)
    {
    ...
    private RedirectToPageResult NotifyAndRedirectToPage<TEntity>(TEntity succ, string pageName, object? routeValues = null) where TEntity : IEntity
    {
    ...
    protected async Task<string> UploadFile<TUploadModel>(string moduleName, string fieldName, string id, IFormFile? formFile, string? fileName = null)
    {
        ...
        if (formFile != null)
        {
            ...
            (await FileUploadHelper.ProcessFormFile<TUploadModel, string>(formFile!,
                permittedExtensions,
                fileSizeLimit,
			    validateActivePdf : Configuration.GetValue<bool>("UsersUpload:ValidateActivePdf"),
