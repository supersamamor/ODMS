...
{   
    ...
    {
		var result = await Mediatr.Send(DataRequest!.ToQuery<GetDashboardThemeQuery>());
    ...
    {        
        return await BatchUploadAsync<IndexModel, DashboardThemeState>(BatchUpload.BatchUploadForm, nameof(DashboardThemeState), "Index");
    }

    public IActionResult OnPostDownloadTemplate()
    {
        ModelState.Clear();
		BatchUpload.BatchUploadFileName = ExcelService.ExportTemplate<DashboardThemeState>(_uploadPath + "\\" + WebConstants.ExcelTemplateSubFolder);
