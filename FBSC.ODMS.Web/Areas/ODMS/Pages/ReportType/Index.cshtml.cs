...
{   
    ...
    {
		var result = await Mediatr.Send(DataRequest!.ToQuery<GetReportTypeQuery>());
    ...
    {        
        return await BatchUploadAsync<IndexModel, ReportTypeState>(BatchUpload.BatchUploadForm, nameof(ReportTypeState), "Index");
    }

    public IActionResult OnPostDownloadTemplate()
    {
        ModelState.Clear();
		BatchUpload.BatchUploadFileName = ExcelService.ExportTemplate<ReportTypeState>(_uploadPath + "\\" + WebConstants.ExcelTemplateSubFolder);
