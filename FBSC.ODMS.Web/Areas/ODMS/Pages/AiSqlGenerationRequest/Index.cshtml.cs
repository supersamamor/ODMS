...
{   
    ...
    {
		var result = await Mediatr.Send(DataRequest!.ToQuery<GetAiSqlGenerationRequestQuery>());
    ...
    {        
        return await BatchUploadAsync<IndexModel, AiSqlGenerationRequestState>(BatchUpload.BatchUploadForm, nameof(AiSqlGenerationRequestState), "Index");
    }

    public IActionResult OnPostDownloadTemplate()
    {
        ModelState.Clear();
		BatchUpload.BatchUploadFileName = ExcelService.ExportTemplate<AiSqlGenerationRequestState>(_uploadPath + "\\" + WebConstants.ExcelTemplateSubFolder);
