...
{   
    ...
    {
		var result = await Mediatr.Send(DataRequest!.ToQuery<GetDashboardRefreshJobQuery>());
    ...
    {        
        return await BatchUploadAsync<IndexModel, DashboardRefreshJobState>(BatchUpload.BatchUploadForm, nameof(DashboardRefreshJobState), "Index");
    }

    public IActionResult OnPostDownloadTemplate()
    {
        ModelState.Clear();
		BatchUpload.BatchUploadFileName = ExcelService.ExportTemplate<DashboardRefreshJobState>(_uploadPath + "\\" + WebConstants.ExcelTemplateSubFolder);
