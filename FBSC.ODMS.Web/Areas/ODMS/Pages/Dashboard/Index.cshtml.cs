...
{   
    ...
    {
		var result = await Mediatr.Send(DataRequest!.ToQuery<GetDashboardQuery>());
    ...
    {        
        return await BatchUploadAsync<IndexModel, DashboardState>(BatchUpload.BatchUploadForm, nameof(DashboardState), "Index");
    }

    public IActionResult OnPostDownloadTemplate()
    {
        ModelState.Clear();
		BatchUpload.BatchUploadFileName = ExcelService.ExportTemplate<DashboardState>(_uploadPath + "\\" + WebConstants.ExcelTemplateSubFolder);
