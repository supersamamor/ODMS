...
namespace FBSC.ODMS.Web.Areas.ODMS.Pages.AuditTrail;
[Authorize]
public partial class IndexModel : BasePageModel<IndexModel>
{
    ...
    private static ChangeHistoryModel MergeChanges(AuditLogViewModel? auditLog, string? oldData, string moduleName, int sequence)
    {
        moduleName += "Id";
    ...
    private static string SplitPascalCaseAndRemoveState(string input)
	{
		...
		var words = RegexSplitPascal().Matches(input)
                         .OfType<Match>()
						 .Select(m => m.Value)
						 .ToList();

		// Remove 'State' if it's the last word
		if (words.Count > 0 && words[^1].Equals("state", StringComparison.CurrentCultureIgnoreCase))
		{
    ...
    private static partial Regex RegexSplitPascal();
}
