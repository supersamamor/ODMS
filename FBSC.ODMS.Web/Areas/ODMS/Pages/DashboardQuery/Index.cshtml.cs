...
{   
    ...
    {
		var result = await Mediatr.Send(DataRequest!.ToQuery<GetDashboardQueryQuery>());
    ...
    {        
        return await BatchUploadAsync<IndexModel, DashboardQueryState>(BatchUpload.BatchUploadForm, nameof(DashboardQueryState), "Index");
    }

    public IActionResult OnPostDownloadTemplate()
    {
        ModelState.Clear();
		BatchUpload.BatchUploadFileName = ExcelService.ExportTemplate<DashboardQueryState>(_uploadPath + "\\" + WebConstants.ExcelTemplateSubFolder);
