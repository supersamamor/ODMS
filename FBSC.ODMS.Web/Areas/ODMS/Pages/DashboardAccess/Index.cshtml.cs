...
{   
    ...
    {
		var result = await Mediatr.Send(DataRequest!.ToQuery<GetDashboardAccessQuery>());
    ...
    {        
        return await BatchUploadAsync<IndexModel, DashboardAccessState>(BatchUpload.BatchUploadForm, nameof(DashboardAccessState), "Index");
    }

    public IActionResult OnPostDownloadTemplate()
    {
        ModelState.Clear();
		BatchUpload.BatchUploadFileName = ExcelService.ExportTemplate<DashboardAccessState>(_uploadPath + "\\" + WebConstants.ExcelTemplateSubFolder);
